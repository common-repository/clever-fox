<?php
function giant_features_setting( $wp_customize ) {
$selective_refresh = isset( $wp_customize->selective_refresh ) ? 'postMessage' : 'refresh';
	...
	$wp_customize->add_setting(
		'feature_hs'
			...
			'default'     	=> '1',	
			'capability'     	=> 'edit_theme_options',
			'sanitize_callback' => 'giant_sanitize_checkbox',
			'priority' => 1,
		)
	);
	 ...
	 */
		...
		$wp_customize->add_setting( 'features_contents', 
			array(
			 ...
			 'transport'         => $selective_refresh,
			 'priority' => 8,
			 'default' => giant_get_features_default()
			)
		);
		...
		//Pro feature
		class Giant_features__section_upgrade extends WP_Customize_Control {
			public function render_content() { 
				$theme = wp_get_theme(); // gets the current theme	
				
			?>
				<a class="customizer_feature_upgrade_section up-to-pro" href="https://www.nayrathemes.com/giant-pro/" target="_blank" style="display: none;"><?php esc_html_e('Upgrade to Pro','clever-fox'); ?></a>
				
			<?php }
		}
		
		$wp_customize->add_setting( 'giant_features_upgrade_to_pro', array(
			'capability'			=> 'edit_theme_options',
			'sanitize_callback'	=> 'wp_filter_nohtml_kses',
			'priority' => 5,
		));
		$wp_customize->add_control(
			new Giant_features__section_upgrade(
			$wp_customize,
			'giant_features_upgrade_to_pro',
				array(
					'section'				=> 'features_setting',
				)
			)
...
add_action( 'customize_register', 'giant_features_setting' );

// features selective refresh
function giant_home_features_section_partials( $wp_customize ){	
	// features title
...
add_action( 'customize_register', 'giant_home_features_section_partials' );
