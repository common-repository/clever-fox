<?php
function giant_service_setting( $wp_customize ) {
$selective_refresh = isset( $wp_customize->selective_refresh ) ? 'postMessage' : 'refresh';
	...
	$wp_customize->add_setting( 
		'service_doc_link' , 
			...
			'giant_sanitize_callback' => 'giant_sanitize_text_field',
		) 
	);
	...
	$wp_customize->add_setting(
		'service_headings'
			,array(
			'capability'     	=> 'edit_theme_options',
			'giant_sanitize_callback' => 'giant_sanitize_text',
			'priority' => 3,
		)
	);
	...
	$wp_customize->add_setting(
		'service_hs'
			...
			'default'     	=> '1',	
			'capability'     	=> 'edit_theme_options',
			'giant_sanitize_callback' => 'giant_sanitize_checkbox',
			'priority' => 1,
		)
	);
	...
	$wp_customize->add_setting(
    	...
    	array(
			...
			'capability'     	=> 'edit_theme_options',
			'giant_sanitize_callback' => 'giant_sanitize_html',
	 ...
	 */
		...
		$wp_customize->add_setting( 'service_contents', 
			array(
			 'giant_sanitize_callback' => 'repeater_giant_sanitize',
			 'transport'         => $selective_refresh,
			 'priority' => 8,
			 'default' => giant_get_service_default()
			)
		);
		...
		//Pro feature
		class Giant_service__section_upgrade extends WP_Customize_Control {
			public function render_content() { 
				$theme = wp_get_theme(); // gets the current theme	
				
			?>
				<a class="customizer_service_upgrade_section up-to-pro" href="https://www.nayrathemes.com/giant-pro/" target="_blank" style="display: none;"><?php esc_html_e('Upgrade to Pro','clever-fox'); ?></a>
		...
		$wp_customize->add_setting( 'service_upgrade_to_pro', array(
			'capability'			=> 'edit_theme_options',
			'giant_sanitize_callback'	=> 'wp_filter_nohtml_kses',
			'priority' => 5,
		));
		$wp_customize->add_control(
			new Giant_service__section_upgrade(
			$wp_customize,
...
add_action( 'customize_register', 'giant_service_setting' );

// service selective refresh
function giant_home_service_section_partials( $wp_customize ){	
	// service title
...
add_action( 'customize_register', 'giant_home_service_section_partials' );

// service title
function giant_service_title_render_callback() {
	return get_theme_mod( 'service_title' );
}

// service subtitle
function giant_service_subtitle_render_callback() {
