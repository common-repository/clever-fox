...
function giant_block_categories( $categories ) {
...
if ( version_compare( $GLOBALS['wp_version'], '5.8-alpha-1', '<' ) ) {
    add_filter( 'block_categories', 'giant_block_categories', 10, 2 );
} else {
    add_filter( 'block_categories_all', 'giant_block_categories', 10, 2 );
}

// Register Blocks
function giant_wp_register_script( $block, $options = array() ) {
    register_block_type( 'info-box/' . $block,
        array_merge( array(
            'editor_script' => 'giant_editor_script',
            'editor_style'  => 'giant_editor_style',
            'script'        => 'giant_script',
            'style'         => 'giant_style',
        ), $options )
    );
...
// Enqueue assets
function giant_enqueue_block_assets() {
    wp_enqueue_script( 'font-awesome-kit', CLEVERFOX_PLUGIN_URL . '/inc/giant/block/assets/js/font-awesome-kit.js', array(), '1.0', true );
}
add_action( 'enqueue_block_assets', 'giant_enqueue_block_assets' );

function giant_register() {
    // Resister script in editor
	wp_register_script( 'giant_editor_script', CLEVERFOX_PLUGIN_URL . 'inc/giant/block/dist/editor.js', array( 'wp-blob', 'wp-block-editor', 'wp-blocks', 'wp-components', 'wp-compose', 'wp-data', 'wp-element', 'wp-html-entities', 'wp-i18n', 'wp-rich-text', 'font-awesome-kit' ), '1.0', false ); 
    wp_register_style( 'giant_editor_style', CLEVERFOX_PLUGIN_URL . 'inc/giant/block/dist/editor.css', array( 'wp-edit-blocks'), filemtime(CLEVERFOX_PLUGIN_DIR . 'inc/giant/block/dist/editor.css') );

    // Register Blocks
    giant_wp_register_script( 'infos', array( 'render_callback' => 'render_giant_infos' ) );
    giant_wp_register_script( 'info', array( 'render_callback' => 'render_giant_info' ) );
}
add_action( 'init', 'giant_register' );

// Generate Styles
class GiantInfoStyleGenerator {
...
// Render Infos
function render_giant_infos($attributes, $content){
    $cId = isset($attributes['cId']) ? esc_attr($attributes['cId']) : '';

    // Generate Styles
    $infosStyle = new GiantInfoStyleGenerator();
...
// Render Info
function render_giant_info( $attributes ) {
    extract( $attributes );
    ...
    // Generate Styles
    $infoStyle = new GiantInfoStyleGenerator();
    $infoStyle::addStyle("#giantInfo-$cId .giantInfo .icon", array(
    ...
    // echo "<div class='av-column-".$columns['desktop']." ".$infoTypes."' id='giantInfo-$cId'>
	...
	echo "<div class='av-column-" . esc_attr($columns['desktop']) . " " . esc_attr($infoTypes) . "' id='giantInfo-" . esc_attr($cId) . "'>
        <aside class='widget widget-contact'>
            <div class='contact-area'>
